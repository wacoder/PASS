...
%     sampling resolution wj_comm 将delay bin的精度设置为系统采样的频率
...
if ~settings.channel_global.noise_on; 
   ...
   stdsettings.channel.noise.type  = 'on'; % cf. signal model (should be off anyway)
...
% normal operation: initialization of tags (query)
   ...
   for i = 1 : settings.readerpool.n
      for j = 1 : settings.tagpool.n
         ...
         settings.channel_rt{i,j}.smallscale.on = true;
      end
   end
   ...
   for i = 1 : settings.tagpool.n
      ...
      %    tag_decoded{i} = temp.decoded; NOT NEEDED 标签接收到得解调信息
