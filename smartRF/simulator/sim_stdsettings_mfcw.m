...
% finished line by jing aug 13th
...
% distance reader to sensatag
stdsettings.dist_rs = []; % m
%   round to sampling resolution
stdsettings.dist_rs = round(stdsettings.dist_rs/settings.c*settings.fs)*settings.c/settings.fs;

% distance sensatag to reader
stdsettings.dist_sr = stdsettings.dist_rs';

% *******************************************************************************************************
% channel tag <-> sensatag
% Sept. 18th 2013, modified by Jing Wang

% distance sensatag to tag
stdsettings.dist_st = []; % m
% round to sampling resolution
stdsettings.dist_st = round(stdsettings.dist_st/settings.c*settings.fs)*settings.c/settings.fs;
% distance tag to sensatag
stdsettings.dist_ts = stdsettings.dist_st;
...
stdsettings.reader.command.m       =      2; % {1, 2, 4, 8} (1:FM0, 2,4,8:Miller)};
...
% state
stdsettings.reader.state           =     ''; % state of reader according to EPCglobal Class1 Gen2 
...
% modulation
stdsettings.tag.modulation.force       =  false; % force modulation / filtering (if tag is not functional)
...
% sensatag identification
stdsettings.sensatag.id.rn16 = '3045'; % RN16 number(hex)
stdsettings.sensatag.id.pc   = '0006'; % protocol-control(PC)bits(hex)
stdsettings.sensatag.id.epc  = '0013 1F4A 0000 0000 0000 0018'; % EPC

% sensatag state 

stdsettings.sensatag.state.epc     = ' '; % state of sensatag according to sensatag protocol

% clock
stdsettings.sensatag.clock.fcenter    = 1.92e6; % Hz center frequency in MHz
stdsettings.sensatag.clock.fsigma     =    5/3; % percent sampling time variation (sigma)
stdsettings.sensatag.clock.phi0       =     -1; % deg phase shift (0<phi0<360); -1:random
stdsettings.sensatag.clock.length     =    NaN; % samples maxiam sample time
stdsettings.sensatag.clock.mode       = 'udef'; % ('fs','fclk')
stdsettings.sensatag.clock.fs         = settings.fs; % Hz sampling frequency

% modulation


stdsettings.sensatag.modulation.pfnc_bias   = 0.98; % bias towards "unmodulated"
stdsettings.sensatag.modulation.t0          = 0;     % s initial delay
stdsettings.sensatag.modulation.lf          = stdsettings.reader.modulation.lf; % Hz backscatter link frequency 
stdsettings.sensatag.modulation.charfile    =  ''; % reflection coeff

% *****
stdsettings.sensatag.modulation.nfilt       = 32; %filterbank channel filter length
stdsettings.sensatag.modulation.nfft        =  4096; % filterbank channels
stdsettings.sensatag.modulation.fclk        = stdsettings.sensatag.clock.fcenter; % Hz tag clock frequency
stdsettings.sensatag.modulation.fc          = stdsettings.reader.oscillator.fcenter; % Hz carrier frequency
stdsettings.sensatag.modulation.fs          = settings.fs;  % Hz sampling frequency

% demodulation
stdsettings.sensatag.demodulation.agc_sat        =           1; % keep functional for Pcarrier, Vdda out of range (AGC)
stdsettings.sensatag.demodulation.vdda           =         NaN; % V power supply (Vdda) voltage (scalar); Vgnd = Agnd = 0 V
stdsettings.sensatag.demodulation.tau            =   169.12e-9; % s time constant of input RC filter
stdsettings.sensatag.demodulation.force_exact_rc =       false; % force recalculation of RC filter coeff. if loaded ones are not an exact match
stdsettings.sensatag.demodulation.agc_len        =         250; % periods of fc window length for AGC power detection (fcn. EST_POWER)
stdsettings.sensatag.demodulation.agc_ol         =          50; % percent overlapping between AGC power detection windows (fcn. EST_POWER)
stdsettings.sensatag.demodulation.agc_nbins      =         100; % number of bins for AGC amplitude histogram
stdsettings.sensatag.demodulation.dvpeak         =       58e-3; % V steady state venv - vpeak
stdsettings.sensatag.demodulation.slewrate       =      .045e6; % V/s maximum slew rate for vpeak
stdsettings.sensatag.demodulation.ar             =        5e-5; % V rise factor for vpeak (empirical)
stdsettings.sensatag.demodulation.af             = stdsettings.sensatag.demodulation.ar*1e-4; % V fall factor for vpeak (empirical)
stdsettings.sensatag.demodulation.debouncelength =           2; % length of debounce filter (MA filter)
stdsettings.sensatag.demodulation.fs             = settings.fs; % Hz sampling frequency
stdsettings.sensatag.demodulation.fc             = stdsettings.reader.oscillator.fcenter; % Hz carrier (center) frequency
stdsettings.sensatag.demodulation.fclk           = stdsettings.sensatag.clock.fcenter; % Hz tag clock (ce     nter) frequency
stdsettings.sensatag.demodulation.vdda           =           2; % voltage of sensatag power 

%     downsampling for performance reasons (use with care!)
stdsettings.sensatag.demodulation.turbo          =     true; % downsampling prior to vpeak calculation if true
stdsettings.sensatag.demodulation.turbo_osfactor =       25; % oversampling rate (critical sampling: 2) for "turbo" downsampling

% encoding
stdsettings.sensatag.encoding.trext = stdsettings.reader.command.trext; % pilot tone on/off
stdsettings.sensatag.encoding.m     = stdsettings.reader.command.m; % (1:FM0, 2,4,8:Miller)

% decoding
stdsettings.sensatag.decoding.fclk = stdsettings.sensatag.clock.fcenter; % Hz center ("clock") frequency in MHz