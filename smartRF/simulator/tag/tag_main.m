...
switch lower(cmd)
   ...
   case 'prep_rn16'
      ...
      if strcmpi(settings.state.epc, 'reply') 
          settings.id.rn16 = num2hexstr(randi(16,1,4)-1); % randi generate number between 1:max 
...
%           settings.id.rn16 = num2hexstr(randi(16,1,4)-1); % randi generate number between 1:max 
%          % encode command and make alphabet binary {0,1} (tag_encoding returns {-1, 1})
%         settings.encdata = [0 0 0 0 0 0 0 0 0];
         settings.encdata = (tag_encoding([0 0 0 0 0 0 0 0 0]', settings.encoding, false) + 1) / 2;
      else
         settings.encdata = [0]; % unmodulated backscatter
      end
      % determine lengths and sanitize settings
