...
function version = gate3d_vissetup(readerpool, tagpool, stagpool, loopsettings, sloopsettings, surfaces, room_dim, visibility)
version = 'beta 3.1';
...
internalsettings.stag_color = [zeros(10,1), ones(10,1), 0.5*ones(10,1)];
...
% sensatag position
for i = 1 : stagpool.n
    stagpos.x{i} = sloopsettings.pos_t{i}(1:sloopsettings.n_sig, 1);
    stagpos.y{i} = sloopsettings.pos_t{i}(1:sloopsettings.n_sig, 2);
    stagpos.z{i} = sloopsettings.pos_t{i}(1:sloopsettings.n_sig, 3);
end
...
% sensatag positions
for i = 1 : stagpool.n
   stag_color = internalsettings.stag_color(1+mod(i-1, size(internalsettings.stag_color,1)), :);
   plot3(stagpos.x{i}, stagpos.y{i}, stagpos.z{i}, '.', 'MarkerFaceColor',stag_color, 'MarkerEdgeColor',stag_color);
   text(stagpool.pos{i}(1)+0.01, stagpool.pos{i}(2)+0.01, stagpool.pos{i}(3)+0.01,...
       sprintf('%s(%i)','s',i),'Fontsize',internalsettings.fontsize);
end
